...
import (
	"encoding/json"
	"fmt"
	"github.com/cosmos/cosmos-sdk/efx/app"
	"io"
	"os"
...
func main() {
	...
	rootDir := os.ExpandEnv("$HOME/.wrappercoind")
...
func newApp(logger log.Logger, db dbm.DB, storeTracer io.Writer) abci.Application {
	return app.NewWrapperApp(logger, db, baseapp.SetPruning(viper.GetString("pruning")))
}

func exportAppStateAndTMValidators(logger log.Logger, db dbm.DB, storeTracer io.Writer) (
	json.RawMessage, []tmtypes.GenesisValidator, error) {
	bapp := app.NewWrapperApp(logger, db)
