...
// accounts already exist.
func SingleInputSendMsg(mapper auth.AccountKeeper, bk wbank.Keeper) simulation.Operation {
	handler := wbank.NewHandler(bk)
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.Account, event func(string)) (action string, fOps []simulation.FutureOperation, err error) {
		fromAcc, action, msg, abort := createSingleInputSendMsg(r, ctx, accs, mapper)
		if abort {
			return action, nil, nil
...
func createSingleInputSendMsg(r *rand.Rand, ctx sdk.Context, accs []simulation.Account, mapper auth.AccountKeeper) (fromAcc simulation.Account, action string, msg wbank.MsgSend, abort bool) {
	...
	msg = wbank.MsgSend{
		Inputs:  []wbank.Input{wbank.NewInput(fromAcc.Address, coins)},
		Outputs: []wbank.Output{wbank.NewOutput(toAddr, coins)},
	}
	return
}
...
func sendAndVerifyMsgSend(app *baseapp.BaseApp, mapper auth.AccountKeeper, msg wbank.MsgSend, ctx sdk.Context, privkeys []crypto.PrivKey, handler sdk.Handler) error {
